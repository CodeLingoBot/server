...
type Action struct {
	...
	Authorized uint8  `orm:"column(authorized)"`
}
...
// Id doesn't exist
func GetActionById(id int) (v *Action, err error) {
	o := orm.NewOrm()
	v = &Action{Id: id}
...
// UpdateAction updates Action by Id and returns error if
// the record to be updated doesn't exist
func UpdateActionById(m *Action) (err error) {
	o := orm.NewOrm()
	v := Action{Id: m.Id}
...
func DeleteAction(id int) (err error) {
	o := orm.NewOrm()
	v := Action{Id: id}
	// ascertain id exists in the database
	if err = o.Read(&v); err == nil {
